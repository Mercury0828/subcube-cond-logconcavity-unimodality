...
### Frozen snapshot 2026-06-22. Read this first to resume.

> **如何冷启动（中文）：** 打开本文件 + `docs/ledger_sr_subcond.md`（详细记录）+ `guide.md`（只读宪法）。
> 当前进度：与 web GPT-5.5-Pro 的攻击循环已到 **round 14（已写好简报、待中转发送）**。下一步动作 =
> 把根目录 **`BRIEF_FOR_PRO.md`** 整段发给 web GPT-5.5-Pro，把它的完整回复贴回来，然后由编排者
> （Claude）派独立审计 + 复核 + 更新 ledger。🔴 仍未闭合、未证明。🔴 owner 曾说"不披露 AI"——本编排
...
- **Current lean: R+ (a poly tester likely exists), ≈70/30**, conditional on one precise open inequality (§5).
- **Round 14 is written + sent (`BRIEF_FOR_PRO.md`), awaiting Pro's reply** — this is the resume point.
...
4. **Rounds 13–14**: closed the channel-degeneracy worry + the degree-2 case, proved several class rates,
   and (referee finding) showed the hardest known obstruction is caught — confining everything to one
   higher-order inequality (§5).
...
- **Referee finding:** `μ_4` (the canonical Rayleigh-but-not-SR example that defeats the *entire* old
  covariance/`V`/interlacer route, worst conditional covariance `=0`) **IS caught by a bounded channel**
  `(0.1,0.55)` (noisy-rank complex roots `−1.90±0.72i`, non-PB, constant margin). ⟹ the hardest known
  higher-order obstruction is bounded-channel-visible — strong evidence R+.

## 5. 🎯 THE ONE OPEN RESIDUAL (what round 14 attacks)
> **(34) general higher-order noise-rank rate.** For arbitrary (nonproduct, nonhomogeneous) stable children,
> **`I_i(h) ≥ η ⟹ NR(h) ≥ poly(η, 1/d)`** — equivalently, a degree-`d` noisy-rank PGF with a complex root
> at imaginary distance `δ` is `≥ poly(δ)`-far from the Poisson-binomial class in TV (a quantitative
> real-rootedness ↔ Poisson-binomial / Pólya-frequency stability theorem), with the witnessing channel bounded.

- **Prove (34) ⟹ R+:** a `poly(n,1/ε)` SUBCOND SR tester (localize → estimate `NR` on the face over the
  bounded-channel box). Complexity `Õ(n^{2+2a+2b}/ε^{2+4a})` for a rate `NR≥c I_i^a/d^b`.
- **Refute (34) ⟹ R−:** a *spread* nonproduct stable-children family whose higher-PF defect evades ALL
  bounded channels (`NR=d^{−ω(1)}` while `I_i=Ω(1/poly)`, all projected ranks PB, evading C5) — a hard instance.
- **Evidence leans R+** (C2/C3 closed unconditionally; product/covariance/homogeneous done; `μ_4` caught).

## 6. HOW TO RESUME (exact next actions)
1. **Relay round 14:** send the entire root **`BRIEF_FOR_PRO.md`** (below the "send as-is" line) to web
   GPT-5.5-Pro (human relay). Paste Pro's full reply back to the orchestrator (Claude Code session).
2. **Orchestrator then:** archive the reply to `docs/rounds/round14_pro_response.md`; spawn 1–2 independent
